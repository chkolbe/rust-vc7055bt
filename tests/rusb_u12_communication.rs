...
mod tests {
    ...
    #[test]
    fn library_dependencies_exist() {
        // This Version reflects the LibUSB Version on the System
        let version = rusb::version();
        assert_eq!(version.major(), 1u16, "rusb Major Version not supported");
        assert_eq!(version.minor(), 0u16, "rusb Minor Version not supported");
    }
    ...
    fn find_labjack_u12() {
        ...
        let mut recv_buffer = [0u8; 8];
        match dh.read_interrupt(0x81u8, &mut recv_buffer, Duration::from_millis(20)) {
            Ok(len) => {
                assert_eq!(len, 8, "Not all Bytes read from Device!");
